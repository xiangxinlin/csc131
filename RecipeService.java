...
public class RecipeService {
    ...
    public List<Recipe> searchRecipesByDiet(DietType dietType) {
        try {
    ...
    private List<Recipe> parseRecipesFromJson(String jsonResponse) {
        ...
        if (jsonObject.has("results")) {
            ...
            for (JsonElement result : resultsArray) {
                ...
                // Assuming all recipes fetched have the same diet type
                DietType dietType = DietType.VEGAN;

                recipes.add(new Recipe(title, ingredients, instructions, dietType));
            }
        }
